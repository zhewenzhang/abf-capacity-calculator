...
16. **Shared Workspace Collaboration (v1.18.0)** — Two or more Google accounts can share a single ABF dataset. Roles: `owner` / `editor` / `viewer`. Workspace switcher lives in the header; UID-based invite (no email magic link). Personal data path (`users/{uid}/...`) is preserved; shared data lives under `workspaces/{wid}/projects/{projectId}/...`. See [docs/WORKSPACE_COLLABORATION.md](docs/WORKSPACE_COLLABORATION.md).
