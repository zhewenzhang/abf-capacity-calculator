...
32. **Abnormality Intelligence Layer (v1.43.0)** — Enhanced abnormality diagnostics with business-aware severity scoring: **ABNORMALITY_TAXONOMY** static registry mapping 20 DQ issue patterns to 6 business categories (data, capacity, sales, BP, scenario, forecast). **Composite severity scoring**: `min(100, round((baseScore + impactBonus) * domainWeight * 10) / 10)` with domain weights (capacity=1.2, bp=1.1, sales=1.0, data=0.9, scenario=0.8, forecast=0.8). **Evidence citations**: Structured `EvidenceCitation[]` with metric/value/threshold/unit/source/period fields. **"Why it matters today" narrative**: Deterministic template generating severity label + domain context + recommended action. **AI Copilot Tool 8**: `explainAbnormalityDetail` with keyword routing (abnormality/異常/issue/problem). 13 new core unit tests. Pure function, zero side effects, no service imports. See [docs/operations/V1_42_TO_V1_45_PRODUCT_PLAN.md](docs/operations/V1_42_TO_V1_45_PRODUCT_PLAN.md).
33. **Operational What-if Scenario (v1.44.0)** — Operational scenario simulation for capacity planning decisions: **CapacityShiftScenario**: Shifts capacity months by N (clamp [-12,+12]), drops out-of-range months. **ForecastAdjustmentScenario**: Adjusts forecastPcs by percentage with customer/SKU/month filters. **OrderDisappearanceScenario**: Removes matching forecasts. **CustomerSkuImpact**: Per-customer and per-SKU revenue deltas, top 20 SKUs. **Entry**: `runOperationalScenario(params)` dispatches to correct handler. **Key design**: Transforms raw data before calling `scenarioEngine.ts` rather than modifying the existing engine. AI Copilot Tool 9: `explainScenarioV2Impact` with keyword routing (scenario v2/營運情境/what-if). 23 new core unit tests. Pure function, zero side effects. See [docs/operations/V1_42_TO_V1_45_PRODUCT_PLAN.md](docs/operations/V1_42_TO_V1_45_PRODUCT_PLAN.md).
34. **Management Report Pack (v1.45.0)** — Deterministic management report generation: **Daily reports** (9 sections: executive summary, data confidence, KPI grid, top risks, required fixes, revenue/BP, look-ahead, scenario comparison, AI narrative draft). **Weekly reports** (14 sections: daily + week-over-week trend, capacity utilization trend, forecast accuracy, customer concentration, SKU portfolio health). **Export**: `exportReportToMarkdown()` and `exportReportToJson()` with UTF-8 BOM for Excel compatibility. **Sensitive key stripping**: `sanitizeObject()` removes apikey/token/password/secret fields. **AI Copilot Tool 10**: `generateReportNarrative` with keyword routing (report/管理報告/daily/weekly). 41 new core unit tests. Pure function, zero side effects. See [docs/operations/V1_42_TO_V1_45_PRODUCT_PLAN.md](docs/operations/V1_42_TO_V1_45_PRODUCT_PLAN.md).
...
- **2026-05-30 v1.51.3**: Synchronized visible app version and package metadata after the v1.51 Designbyte + DeepSeek release.
- **2026-05-30 v1.51.2**: Release gate workflow — merged v1.48 demo assets, completed main sync, demo seed validation, test/lint/build verification, and deployed to Firebase Hosting.
- **2026-05-30 v1.51.1**: DeepSeek provider wiring fix — session key wiring from AiProviderSettingsDrawer to CopilotChat, runtime DeepSeek call path, output validation integration, deterministic fallback.
- **2026-05-30 v1.51.0**: Designbyte theme integration and DeepSeek AI provider — CSS token mapping for elegant professional theme, DeepSeek v4 Flash provider with session-only BYOK key, 14 provider tests, i18n support.
- **2026-05-28 v1.45.0**: Management Report Pack — Deterministic management report generation for daily/weekly operations. **Daily reports** (9 sections): executive summary, data confidence, KPI grid, top risks, required fixes, revenue/BP status, look-ahead highlights, scenario recommendations, AI narrative draft. **Weekly reports** (14 sections): daily sections + week-over-week trend, capacity utilization trend, forecast accuracy, customer concentration, SKU portfolio health. **Export formats**: Markdown and JSON (with UTF-8 BOM for Excel compatibility). **Sensitive key stripping**: `sanitizeObject()` removes apikey/token/password/secret fields from exports. **AI Copilot Tool 10**: `generateReportNarrative` deterministic tool with keyword routing (report/管理報告/daily/weekly). **Core pure function**: `buildManagementReport()` with zero side effects, fixed precision (toFixed(1)), stable sort, no Date.now()/Math.random(). 41 new core unit tests. 1398 total tests passing, 0 lint errors, clean build. No external AI API, no Firestore writes, no formula changes. See [docs/operations/V1_42_TO_V1_45_PRODUCT_PLAN.md](docs/operations/V1_42_TO_V1_45_PRODUCT_PLAN.md).
- **2026-05-28 v1.44.0**: Operational What-if Scenario — Operational scenario simulation for capacity planning decisions. **CapacityShiftScenario**: Shifts capacity months by N (clamp [-12,+12]), drops out-of-range months, preserves baseline. **ForecastAdjustmentScenario**: Adjusts forecastPcs by percentage with optional customer/SKU/month filters. **OrderDisappearanceScenario**: Removes matching forecasts by customer/SKU/month. **CustomerSkuImpact**: Per-customer and per-SKU revenue deltas with top 20 SKUs by absolute delta. **AI Copilot Tool 9**: `explainScenarioV2Impact` deterministic tool with keyword routing (scenario v2/營運情境/what-if/operational). **Key design**: Transforms raw data before calling `scenarioEngine.ts` rather than modifying the existing engine; uses `buildComparisonFromTransformedData()` to manually construct ScenarioComparison. 23 new core unit tests. Pure function, zero side effects, no service imports. No external AI API, no Firestore writes, no formula changes. See [docs/operations/V1_42_TO_V1_45_PRODUCT_PLAN.md](docs/operations/V1_42_TO_V1_45_PRODUCT_PLAN.md).
- **2026-05-28 v1.43.0**: Abnormality Intelligence Layer — Enhanced abnormality diagnostics with business-aware severity scoring. **ABNORMALITY_TAXONOMY**: Static registry mapping 20 DQ issue patterns to 6 business categories (data, capacity, sales, BP, scenario, forecast) with base scores and impact bonuses. **Composite severity scoring**: `min(100, round((baseScore + impactBonus) * domainWeight * 10) / 10)` with domain weights (capacity=1.2, bp=1.1, sales=1.0, data=0.9, scenario=0.8, forecast=0.8). **Evidence citations**: Structured `EvidenceCitation[]` with metric/value/threshold/unit/source/period fields derived from DQ issue evidence. **"Why it matters today" narrative**: Deterministic template generating severity label + domain context + recommended action. **AI Copilot Tool 8**: `explainAbnormalityDetail` with keyword routing (abnormality/異常/issue/problem/diagnostic). 13 new core unit tests. Pure function, zero side effects, no service imports. No external AI API, no Firestore writes, no formula changes. See [docs/operations/V1_42_TO_V1_45_PRODUCT_PLAN.md](docs/operations/V1_42_TO_V1_45_PRODUCT_PLAN.md).
- **2026-05-28 v1.42.0**: Daily Operations Workbench — New `/operations` page serving as the daily entry point for capacity planners. **Workflow pipeline stepper**: 7 stages (Products, Forecasts, Capacity, Parameters, BP Targets, Analysis, Scenario) with derived status (ready/warning/blocked/notStarted) from data quality and data presence. **Abnormality summary**: Cross-domain issue classification (data, capacity, sales, BP, scenario) sorted by severity with top issues per domain. **Look-Ahead Focus panel**: Next 6 months utilization and shortage forecast. **Revenue/BP summary**: Current forecast vs BP target with attainment status. **Scenario shortcuts**: 5 presets (volume up/down, capacity up, price up, stress test) with URL param navigation to `/scenario`. **AI Copilot Tool 7**: `explainWorkbenchOverview` deterministic tool for workbench state explanation with keyword routing (workbench/overview/operations/工作台/總覽). **Core pure function**: `buildWorkbenchViewModel` with zero side effects, no service imports, delegates to existing `dataQuality`, `analytics`, `bpTargets` modules. 55 new core unit tests + 18 page render tests. 1321 total tests passing, 0 lint errors, clean build. No external AI API, no Firestore writes, no formula changes, no new npm dependencies. See [docs/workbench/V1_42_DAILY_OPERATIONS_WORKBENCH_PRODUCT_SPEC.md](docs/workbench/V1_42_DAILY_OPERATIONS_WORKBENCH_PRODUCT_SPEC.md), [docs/workbench/V1_42_DAILY_OPERATIONS_WORKBENCH_ARCHITECTURE.md](docs/workbench/V1_42_DAILY_OPERATIONS_WORKBENCH_ARCHITECTURE.md).
- **2026-05-27 v1.41.0**: AI Copilot Reliability Marathon — Hardened the AI Data Copilot across reliability, transparency, and test coverage dimensions. **Output validation wiring**: `validateProviderOutput` confirmed wired into all provider response paths (local/mock/external-byok) with regression tests preventing future disconnection. **UX transparency**: Answer status tags (Deterministic/Blocked/Mock/Warning/Needs External AI) with priority ordering; "Why this answer?" collapsible section showing tool used, data analyzed, caveats, and validation status. **Q&A Router v2**: Expanded keyword routing with Traditional Chinese support (資料品質/產能/瓶頸/短缺/差距/達成/情境/前瞻/未來/修復); improved fallback messages listing available question types. **Red Team Corpus 100**: 100 evaluation test cases across 8 categories (prompt injection, fake save claims, data fabrication, currency confusion, causality claims, PII leak, viewer bypass, provider unsafe output) with automated eval runner. **Security boundary tests**: Comprehensive verification of no network calls, no storage access, no save imports, no external URLs in copilot modules. **i18n parity**: 12 new copilot i18n keys in EN + zh-TW. **Validation pattern fix**: `auto-save` regex now correctly matches `auto-saved` variant. 1249 tests passing, 0 lint errors, clean build. No external AI API, no Firestore writes, no formula changes, no npm dependency additions. See [docs/ai-copilot/V1_41_AI_COPILOT_RELIABILITY_IMPLEMENTATION.md](docs/ai-copilot/V1_41_AI_COPILOT_RELIABILITY_IMPLEMENTATION.md), [docs/ai-copilot/V1_41_AI_COPILOT_BROWSER_SMOKE_TEST.md](docs/ai-copilot/V1_41_AI_COPILOT_BROWSER_SMOKE_TEST.md).
