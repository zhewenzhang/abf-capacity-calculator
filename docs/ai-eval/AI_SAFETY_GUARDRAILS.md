...
* **規範**：當 `confidenceLevel = "low"`（對應系統底層 `confidenceScore` 數字在 `0-59` 區間）時，AI 必須在報告頂部進行醒目的“低信心警示”，且分析語氣必須全部降級為“邏輯推演”、“僅供參考”。嚴禁在 Low 信心下使用高強度肯定語氣得出確定性業務結論。
...
AI 必須根據系統的 `confidenceLevel` 級別（對應系統底層 `confidenceScore` 數字區間），動態調整報告的語氣強度：
