...
## 4. v1.22.1 Snapshot Permission Hardening 實現記錄

### 🔒 Firestore Rules 實現 (2026-05-24)

v1.22.1 在 `firestore.rules` 中為 snapshots 添加了專用規則，確保：

#### Personal Snapshots (`users/{uid}/projects/{projectId}/snapshots/{snapshotId}`)
...
#### Workspace Snapshots (`workspaces/{workspaceId}/projects/{projectId}/snapshots/{snapshotId}`)
...
1. **Immutable 保證**: `allow update: if false;` 確保任何人都無法修改已建立的快照。
2. **Editor 刪除限制**: Editor 只能刪除 `resource.data.createdBy == request.auth.uid` 的快照，防止惡意刪除他人快照。
