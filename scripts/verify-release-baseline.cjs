...
const PAGE_FILES = ['App.tsx', 'pages/DailyOperationsWorkbench.tsx', 'pages/CalculationResults.tsx',
...
const CURRENCY_PATTERNS = [
  { pattern: 'M TWD', label: 'M TWD (should use M NTD for default display)' },
  { pattern: 'M CNY', label: 'M CNY (should use M NTD)' },
  { pattern: 'K TWD', label: 'K TWD' },
  { pattern: 'B TWD', label: 'B TWD' },
  { pattern: 'NT$', label: 'NT$ (should use NTD)' },
];
...
for (const pf of PAGE_FILES) {
  ...
  for (const cp of CURRENCY_PATTERNS) {
    // Skip ¥ check for App.tsx (currency selector is allowed) — commented: skip ¥ entirely, it's too noisy
    if (pageContent.includes(cp.pattern)) {
      errors.push(`WARN: ${pf} contains ${cp.label} (${JSON.stringify(cp.pattern)}) — verify if this is intentional default display`);
      // Warnings don't fail the build, but must be reviewed
    }
  }
}
